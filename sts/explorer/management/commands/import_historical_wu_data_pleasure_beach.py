...
class Command(BaseCommand):
	...
	def get_precip_data(self):
		...
		stations = WeatherStations.objects.filter(id__in=[1398])
		#between today and 2004
		latest = datetime(2017, 4, 24)
		earliest = datetime(2015, 6, 1)
		counter = 0

		for dateEval in rrule(MONTHLY, dtstart=earliest, until=latest):
