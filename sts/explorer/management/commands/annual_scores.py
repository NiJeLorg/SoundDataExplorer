	...
	def calc_pass_fail(self):
		...
		for dateEval in rrule(YEARLY, dtstart=earliest, until=today):
			...
			for beach in beaches:
				...
				MaxValueDry = 0
				MaxValueWet = 0
				samples = BeachWQSamples.objects.filter(BeachID__exact=beach, StartDate__gte=firstOfYear, StartDate__lte=lastOfYear)
				...
				if NumberOfSamples > 0:
					for sample in samples:
						...
						if sample.CharacteristicName != 'Total Coliform':
							...
							# do some counting
							if precip['PrecipitationIn__sum'] >= 0.25:
							...
							if sample.CharacteristicName == 'Enterococcus':
								...
								if precip['PrecipitationIn__sum'] >= 0.25:
									if MaxValueWet < sample.ResultValue:
										MaxValueWet = sample.ResultValue
								else:
									if MaxValueDry < sample.ResultValue:
										MaxValueDry = sample.ResultValue
				...
				# write to database
				updated_values = {'NumberOfSamples': TotalNumberOfSamples, 'TotalPassSamples': TotalPassSamples, 'TotalDryWeatherSamples': TotalDryWeatherSamples, 'DryWeatherPassSamples': DryWeatherPassSamples, 'TotalWetWeatherSamples': TotalWetWeatherSamples, 'WetWeatherPassSamples': WetWeatherPassSamples, 'MaxValueWet': MaxValueWet, 'MaxValueDry':MaxValueDry}

				obj, created = AnnualScores.objects.update_or_create(BeachID=beach, Year=firstOfYear, defaults=updated_values)

	def handle(self, *args, **options):
		print "Calculating Annual Sample Pass/Fail...."
		self.calc_pass_fail()
		print "Done."
